...
dotenv.config();
const handleProcessPdf = (req, res) =>
  __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
      ...
      if (SELECTOR && baseURI && API_ENDPOINT && TO_MAILS) {
        ...
        pdfDoc.pipe(
          fs_1.default
            .createWriteStream("LatestArticles.pdf")
            .on("error", (err) => {
              console.log("err", err);
            })
        );
        ...
        reqObj.append(
          "attachment",
          fs_1.default.createReadStream("./LatestArticles.pdf")
        );
        ...
        // axios
        //   .request(config)
        //   .then(({data}:{data:SuccessfulResponse}) => {
        //     res.json(data);
        //   })
        //   .catch(({data}:{data:FailureResponse}) => {
        //     res.json(data.status?data.status:data.status);
        //   });
      } else {
